...
- **v5.8 (French):** `--fuse_contractions` keeps `du`/`des` fused for parsing
  ...
  parser tokenise first even when TreeTagger is used, adding TreeTagger's
  pos/lemma as supplementary `tagger_pos`/`tagger_lemma` columns. CoNLL-U
  headers gained `# child`/`# project`; several CHAT-cleaning and
  metadata-desync bug fixes.
- **v5.7:** `--pos_output`/`--pos_utterance` match the parser's universal POS
  (UPOS) by default rather than the tagger's own tag, so the same regex works
  across corpora and tagger models; `--use_tagger_pos` restores the old
  behaviour.
